...
public:
    ...
    std::list<Unit*> SelectNearbyTargetsNoTotemTarget(float dist) const;
    ...
    void ApplyCooldownAndAmplitudeReduction();
    ...
    static int32 DealHeal(Unit* healer, Unit* victim, uint32 addhealth, SpellInfo const* spellInfo);
    ...
    SpellCastResult CastCustomSpellTrigger(uint32 spellId, SpellValueMod mod, int32 value, Unit* target, TriggerCastFlags triggerFlags, Item* castItem = nullptr, AuraEffect const* triggeredByAura = nullptr, ObjectGuid originalCaster = ObjectGuid::Empty);
    ...
    SpellCastResult CastCustomSpellPct(uint32 spellId, SpellValueMod mod, int32 value, int32 pct, bool scale, bool overlap, bool refresh, uint32 max, Unit* victim, Item* castItem = nullptr, AuraEffect const* triggeredByAura = nullptr, ObjectGuid originalCaster = ObjectGuid::Empty);
    ...
    [[nodiscard]] std::vector<AuraEffect*> GetAurasEffect(AuraType type, SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, ObjectGuid casterGUID = ObjectGuid::Empty) const;
    ...
    [[nodiscard]] SpellSchools  GetHighestCritChanceSpellSchool(SpellSchoolMask mask) const;
    ...
    int32 CalculateSpellDamageWithRatio(SpellSchoolMask schoolMask, float ratio);
