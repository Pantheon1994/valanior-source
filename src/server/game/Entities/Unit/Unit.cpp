...
SpellCastResult Unit::CastCustomSpellTrigger(uint32 spellId, SpellValueMod mod, int32 value, Unit* target, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    CustomSpellValues values;
    values.AddSpellMod(mod, value);
...
bool Unit::CheckIfIsSpellMissWithAttacker(Unit const* attacker, Unit const* victim)
{
    int32 levelDiff = int32(victim->getLevelForTarget(attacker)) - int32(attacker->getLevelForTarget(victim));
    float chance = levelDiff > 4 ? 5.0f + levelDiff * 1.0f : 0.0f; // Base 5% miss chance + 0.1% per level difference
    chance = std::max<float>(chance, 0.0f); // Minimum miss chance is 0%
    chance = std::min<float>(chance, 100.f); // Maximum miss chance is 95%
    return roll_chance_f(chance);
}
...
uint32 Unit::GetDiseasesByCaster(ObjectGuid casterGUID, uint8 mode)
{
    ...
    {
        SPELL_AURA_PERIODIC_DAMAGE, // Frost Fever and Virulent Plague
        SPELL_AURA_PERIODIC_LEECH,  // Blood Plague
        ...
        case SPELLFAMILY_SHAMAN:
            {
                    ...
                    case 33757:
                        {
                            ...
                            player->AddSpellCooldown(dummySpell->Id, 0, 100);
                            int32 attacks = 2;
                ...
                if (dummySpell->Id == 58792)
                {
                    ...
                    float spellPowerRatio = sSpellMgr->AssertSpellInfo(58788)->GetEffect(EFFECT_0).CalcValue();
                    float attackPowerRatio = sSpellMgr->AssertSpellInfo(58788)->GetEffect(EFFECT_1).CalcValue();
                    ...
                    float add_attackpower = (float)(GetTotalAttackPowerValue(BASE_ATTACK));

                    // 1.3speed = 5%, 2.6speed = 10%, 4.0 speed = 15%, so, 1.0speed = 3.84%
                    ApplyPct(add_spellpower, spellPowerRatio);
                    ApplyPct(add_attackpower, attackPowerRatio);
                    ApplyPct(add_spellpower, 1);
                    ApplyPct(add_attackpower, 1);
                    ...
                    if (castItem->GetSlot() == EQUIPMENT_SLOT_OFFHAND && procFlag & PROC_FLAG_DONE_OFFHAND_ATTACK)
                    {
                        ...
                        float spellPowerAmount = add_spellpower * BaseWeaponSpeed;
                        float attackPowerAmount = add_attackpower * BaseWeaponSpeed;

                        basepoints0 = int32(spellPowerAmount + attackPowerAmount);
