...
public:
    ...
    bool TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options = 0, Unit* target = nullptr, bool newInstance = false, uint32 mythicLevel = 0);
    ...
    void learnSpell(uint32 spellId, bool temporary = false, bool learnFromSkill = false, bool sendPacket = true);
    ...
    void SetSpellCooldown(uint32 spellId, int32 cooldown);
    void SendSpellCooldown(uint32 spellId, int32 cooldown);
