...
// 
class spell_item_circle_of_flame : public AuraScript
{
    PrepareAuraScript(spell_item_circle_of_flame);
    ...
    void HandlePeriodic(AuraEffect const* aurEff)
    {
        ...
        if (caster->GetHealthPct() < 2 * aurEff->GetAmount())
            return;

        int32 amount = CalculatePct(aurEff->GetAmount(), caster->GetMaxHealth());
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_item_circle_of_flame::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
};

// 17252 - Mark of the Dragon Lord - ITEM : 13143
class spell_item_mark_of_dragon_lord : public AuraScript
{
    PrepareAuraScript(spell_item_mark_of_dragon_lord);
    ...
    void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (caster->GetPowerPct(POWER_MANA) < GetEffect(EFFECT_2)->GetAmount())
            return;
    ...
    void Register() override
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_item_mark_of_dragon_lord::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
        OnEffectAbsorb += AuraEffectAbsorbFn(spell_item_mark_of_dragon_lord::Absorb, EFFECT_0);
    }
};
...
enum ForceOfWill
{
    ITEM_FORCE_OF_WILL_BUFF = 15595,
};

// 15594 - Force of Will - ITEM : 11810
class spell_item_force_of_will : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 amount = CalculatePct(aurEff->GetAmount(), caster->GetStat(STAT_STAMINA));
        caster->CastCustomSpell(ITEM_FORCE_OF_WILL_BUFF, SPELLVALUE_BASE_POINT0, amount, caster, TRIGGERED_FULL_MASK);
    }
...
// 15604 - Second Wind - ITEM : 11819
class spell_item_second_wind : public AuraScript
{
    PrepareAuraScript(spell_item_second_wind);
    ...
    void HandlePeriodic(AuraEffect const* aurEff)
    {
        ...
        int32 amount = CalculatePct(aurEff->GetAmount(), caster->GetMaxPower(POWER_MANA));
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_item_second_wind::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
void AddSC_item_spell_scripts()
{
    ...
    RegisterSpellScript(spell_item_circle_of_flame);
    RegisterSpellScript(spell_item_mark_of_dragon_lord);
